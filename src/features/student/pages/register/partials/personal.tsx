...
import { get as getPersonal, store as storePersonal, update as updatePersonal } from "@/features/student/services/personal";
...
const StudentPersonal = () => {
    ...
    useEffect(() => {
        getPersonal<StudentPersonalType>({ userId: user?.id }).then((resp) => {
            if (resp.length > 0) {
                const result = resp[0]
