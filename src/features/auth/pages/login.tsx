...
import AuthFooter from "@/components/layout/footer/auth";
import {
    Block,
    BlockContent,
    BlockDes,
...
import { login as loginUser } from "@/features/auth/services/auth";
import { useAuthContext } from "@/hooks/useAuthContext";
import { Navigate } from "react-router";

const Login = () => {
    const { isLogged, setUser, setIsLogged } = useAuthContext()
        ...
        await loginUser(value).then((resp) => {
            if (resp !== undefined) {
                if (resp.user?.phone_verified_at === null) {
                    ...
                    setUser(resp.user);
                    localStorage.setItem('tempUser', JSON.stringify(resp.user))
                } else {
                    setUser(resp.user);
                    setIsLogged(true);
                    navigate('/dashboard');
